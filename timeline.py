...
pestructure = {
    ...
    'of': 'where p.created_by = u.id and u.id = %s and p.last_modified > %s and p.last_modified < %s order by p.last_modified ',
...
def getpost(id):
    ...
    c.execute(
        ...
        where p.created_by = u.username and p.id = %s
        ''', (id, )
    )
    return c.fetchone()

def getpost(profile_id, limit_d, limit_t, way):
    db, c = get_db()
    c.execute(
        pestructure['base'] + pestructure['of'] + pestructure[way],
        (profile_id, limit_d, limit_t)
    )
    return c.fetchall()

def getpost(limit_t, limit_d, way):
    db, c = get_db()
    c.execute(
